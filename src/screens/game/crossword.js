...
export default class Crossword extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      list: '',
      listFix: [],
      xwordsid: this.props.navigation.getParam('id', 0)
    };
  }
  ...
  async _getData() {
    try {
      const response = await axios.get(`${URL.URL}crosswords/${this.state.xwordsid}/answers`, {
        headers: {
      ...
      >
        <Content style={{ flexDirection: 'column' }}>
          ...
          <View style={styles.header}>
            ...
            <View style={styles.headerBox}>
              <View style={styles.menuHeader}>
                <Text style={styles.menuHeaderText}>Petunjuk</Text>
              </View>
              <View style={styles.menuItem}>
                <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 45 }}>
                  {this.state.answer.map((data, index) => {
                    return (
                      <Text key={index}>
                        Soal {data.type}: {data.question}
                      </Text>
                    );
                  })}
                </ScrollView>
